...
const Lainnya = () => {
  ...
  // api url
  const API_URL = import.meta.env.VITE_API_URL_LAINNYA;
  ...
  const handleDelete = async (id) => {
    ...
    if (!isConfirmed) {
      return; // Jika pengguna membatalkan, tidak lanjutkan proses
    }

    try {
      await axios.delete(`${API_URL}/${id}`);
      setData(data.filter((item) => item.id !== id)); // Perbarui data di frontend
    ...
    <div>
      <div className="w-full">
        ...
        <div className="flex flex-col  justify-center p-10 lg:mt-20 md:mt-16 mt-10">
          <h1 className="text-center lg:text-4xl md:text-3xl text-2xl">Berita Lainnya</h1>
